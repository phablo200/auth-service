...
```bash
cp infra/aws/terraform/environments/prod/terraform.tfvars.example infra/aws/terraform/environments/prod/terraform.tfvars
```
...
```bash
terraform -chdir=infra/aws/terraform/environments/prod output alb_dns_name
```
...
The application database URL secret is also generated by Terraform and injected into ECS as `DATABASE_URL`:

- `auth-service-prod/database-url`
...
```bash
terraform -chdir=infra/aws/terraform/environments/prod output alb_url
